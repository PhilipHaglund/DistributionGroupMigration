...
Task CoreStageFiles -requiredVariables ModuleOutDir, SrcRootDir {
    ...
    $Classes = Get-ChildItem -Path "$SrcRootDir\Classes" -Filter *.ps1 | Sort-Object -Property Name
    $PrivateFunctions = Get-ChildItem -Path "$SrcRootDir\Private" -Filter *.ps1
    $PublicFunctions = Get-ChildItem -Path "$SrcRootDir\Public" -Filter *.ps1
    if ($null -ne $Classes) {
        $null = $PSM1Content.Add($Classes)
    }
    if ($null -ne $PrivateFunctions) {
        $null = $PSM1Content.Add($PrivateFunctions)
    }
    ...
    if ($null -eq $PublicFunctions) {
        Update-ModuleManifest -Path ('{0}\{1}.psd1' -f $ModuleOutDir, $ModuleName)
    }
    else {
